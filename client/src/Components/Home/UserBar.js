...
function UserBar() {
    ...
    const favoriteMap = user ?
        user?.favorites.map(fav => 
            <Link to={`/kairos/property/${fav.property?.id}`} 
                key={fav.id} 
    ...
    return (
            <div className=''>
                ...
                <div className='ml-5'>
                    ...
                    <Link className=' text-3xl mt-3 mb-3 underline text-blue-400 hover:text-blue-600' to={'/kairos/profile'}>{user?.username}</Link>
                </div>
                <div className='border-t mt-8'></div>
                <div className='mx-5 mt-10'>  
                    <p className='text-md mb-2 uppercase tracking-wider font-medium'>Current Listing:</p>
                    {user?.property ?
                    <Link className='text-sm mb-3 underline text-gray-500 hover:text-blue-600' to={`/kairos/property/${user?.property?.id}`}>{user?.property.address}</Link> : <p>No Listing</p> }
                </div>
                <div className='ml-5'>
            ...
            <div className='mx-5 mb-3'>
                <p className='text-xl font-semibold mt-10 mb-3'>Welcome!</p>
                <Link className=' text-2xl mt-3 underline text-blue-200' to={'/kairos/'}>Please Log In</Link>
            </div>
            <div className='mx-5'>
                <p className='mt-8'></p>
